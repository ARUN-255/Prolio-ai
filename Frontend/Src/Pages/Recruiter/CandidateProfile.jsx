  ...
  const analyzeProject = async (project) => {
    ...
    try {
      ...
      const data = await analyzePublicProject(project.id, {
        ...
        required_skills: analysisContext.required_skills
          .split(",")
    ...
    <div className="recruiter-page candidate-profile-page">
      ...
      <section className="recruiter-dashboard-grid candidate-profile-grid">
            ...
            <div className="candidate-skills candidate-profile-skills">
              {skills.map((skill, index) => (
                <span key={`${skill.name || skill}-${index}`}>
                  ...
                  {typeof skill === "object" && skill.proficiency ? ` · ${skill.proficiency}` : ""}
                </span>
              ))}
            </div>
        ...
        <article className="recruiter-panel">
          ...
          {resumes.length ? (
            <div className="candidate-resume-list">
              {resumes.map((resume) => (
                <article key={resume.id} className="candidate-resume-item">
                  <div><FileText size={17} /><div><strong>{resume.title}</strong><span>{resume.is_primary ? "Primary resume" : "Public resume"}</span></div></div>
                  <button type="button" disabled={!resume.pdf_url || downloadId === resume.id} onClick={() => downloadResume(resume)}>
                    {downloadId === resume.id ? <LoaderCircle className="spin" size={16} /> : <Download size={16} />}
                  </button>
                </article>
              ))}
              <Link className="candidate-compare-link" to="/recruiter/compare">Compare public resumes →</Link>
            </div>
          ) : <p className="recruiter-small-state">No public resumes.</p>}
        </article>
      </section>
          ...
          <div className="candidate-detail-list">
            {education.map((item, index) => (
              <div className="candidate-detail-item" key={index}>
                ...
                {(item.start_year || item.end_year) && <small>{[item.start_year, item.end_year].filter(Boolean).join(" – ")}</small>}
            ...
            {experiences.map((item, index) => (
              <div className="candidate-detail-item" key={index}>
                ...
                {(item.start_date || item.end_date || item.is_current) && (
                  <small>{[item.start_date, item.is_current ? "Present" : item.end_date].filter(Boolean).join(" – ")}</small>
                )}
                {item.description && <p>{item.description}</p>}
              </div>
            ))}
      ...
      <section className="recruiter-panel">
        <div className="candidate-project-heading">
          <div><h2>Projects</h2><p>Optionally add job context before running the project analyzer.</p></div>
        </div>
        {projects.length > 0 && (
          <div className="project-analysis-context">
            <input value={analysisContext.job_title} onChange={(e) => setAnalysisContext({ ...analysisContext, job_title: e.target.value })} placeholder="Job title (optional)" />
            <input value={analysisContext.required_skills} onChange={(e) => setAnalysisContext({ ...analysisContext, required_skills: e.target.value })} placeholder="Required skills, comma separated" />
            <textarea rows="3" value={analysisContext.job_description} onChange={(e) => setAnalysisContext({ ...analysisContext, job_description: e.target.value })} placeholder="Job description (optional)" />
          </div>
        )}
        {projects.length ? (
          <div className="candidate-project-grid">
            {projects.map((project, index) => {
              const result = projectAnalyses[project.id];
              const deterministic = result?.analysis;
              const ai = result?.ai_analysis;

              return (
                <article className="candidate-project-card" key={project.id || index}>
                  <h3>{project.title || "Project"}</h3>
                  {project.description && <p>{project.description}</p>}
                  <div className="candidate-skills">{safeArray(project.tech_stack).map((tech) => <span key={tech}>{tech}</span>)}</div>
                  <div className="candidate-project-actions">
                    {project.link && <a href={project.link} target="_blank" rel="noreferrer">Open project <ExternalLink size={14} /></a>}
                    <button type="button" disabled={analyzingProjectId === project.id} onClick={() => analyzeProject(project)}>
                      {analyzingProjectId === project.id ? <LoaderCircle className="spin" size={14} /> : <Sparkles size={14} />} Analyze
                    </button>
                  </div>
                  {result && (
                    <div className="project-analysis-result">
                      {deterministic && <p><strong>Technical match:</strong> {deterministic.match_percentage ?? deterministic.score ?? "Analyzed"}{typeof deterministic.match_percentage === "number" ? "%" : ""}</p>}
                      {ai && <><p><strong>AI quality:</strong> {ai.project_quality || "Analyzed"}</p>{ai.recruiter_feedback && <p>{ai.recruiter_feedback}</p>}{safeArray(ai.technical_strengths).length > 0 && <small>Strengths: {ai.technical_strengths.join(" · ")}</small>}{safeArray(ai.technical_gaps).length > 0 && <small>Gaps: {ai.technical_gaps.join(" · ")}</small>}</>}
                      {!result.ai_available && <small>AI feedback was unavailable; deterministic analysis completed.</small>}
                    </div>
                  )}
                </article>
              );
          ...
          <div className="candidate-certificate-grid">
            {certificates.map((item, index) => (
              <article className="candidate-certificate-card" key={index}>
                <div><strong>{item.title || "Certificate"}</strong>{item.issuer && <span>{item.issuer}</span>}{item.date && <small>{String(item.date).slice(0, 10)}</small>}</div>
                {item.file_url && <a href={item.file_url} target="_blank" rel="noreferrer" aria-label="Open certificate"><ExternalLink size={16} /></a>}
              </article>
            ))}
          </div>
